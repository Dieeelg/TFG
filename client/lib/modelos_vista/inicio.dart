...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<void> cargarDatosHome() async {
    ...
    try {
      ...
      if (pautaBD.isNotEmpty) {
        ...
        pautaSemanal = pautaBD.map((dia) => PautaToma(
          data: dia.data,
          dia: "${dia.diaSemanaTexto.substring(0,3)} ${dia.dia}",
          dose: dia.eControl ? "CTRL" : (dia.dose == '0' ? "NON" : (dia.dose ?? "0")),
          estado: estados[dia.data] == 'PENDENTE' && dia.data.compareTo(hoxe) < 0
              ? 'NON_TOMADA'
  ...
  Future<void> confirmarTomaHoxe() async {
    ...
    if (horaConfigurada != null) {
      ...
      if (partes.length == 2) {
        final prevista = DateTime(agora.year, agora.month, agora.day,
            int.tryParse(partes[0]) ?? 0, int.tryParse(partes[1]) ?? 0);
  ...
  Future<void> confirmarToma(String tokenCoidador, String payload) async {
    ...
    bool ok = await _apiService.enviarNotificacion(
